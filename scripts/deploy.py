...
def deploy_token_farm_and_dapp_token(front_end_update=False):
    ...
    tx = penwuin_token.transfer(
        token_farm.address,
        penwuin_token.totalSupply() - KEPT_BALANCE,
        {"from": account},
    )
    tx.wait(1)
    # penwuin_token, weth_token, fau_token/dai
    weth_token = get_contract("weth_token")
    fau_token = get_contract("fau_token")
    dict_of_allowed_tokens = {
        penwuin_token: get_contract("dai_usd_price_feed"),
    ...
    if front_end_update:
        update_front_end()
    return token_farm, penwuin_token
